...
_TOOLS: tuple[dict[str, Any], ...] = (
    {
        "name": "rpr.get_status",
        "description": "Return read-only RPR database status and pathway counts.",
        "inputSchema": {"type": "object", "properties": {}, "additionalProperties": False},
    },
    {
        "name": "rpr.list_pathways",
        "description": "List RPR pathways without changing runtime state.",
        "inputSchema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": "Return one RPR pathway definition and current state.",
        "inputSchema": {
            "type": "object",
            "properties": {"pathway_id": {"type": "string", "minLength": 1}},
            "required": ["pathway_id"],
            "additionalProperties": False,
        },
    },
    {
        "name": "rpr.get_evidence",
        "description": "Return retained evidence events for one RPR pathway.",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "rpr.list_unresolved",
        "description": "List pathways that are not completed, denied, or aborted.",
        "inputSchema": {
            "type": "object",
            "properties": {"limit": {"type": "integer", "minimum": 1, "maximum": 1000}},
            "additionalProperties": False,
...
class ReadOnlyRprMcpServer:
    ...
    def handle(self, request: object) -> dict[str, Any] | None:
        ...
        if not isinstance(method, str) or not method:
            raise JsonRpcError(-32600, "method must be a non-empty string")
    ...
    def _initialize(params: object) -> dict[str, Any]:
        ...
        if requested != STABLE_PROTOCOL_VERSION:
            raise JsonRpcError(
                -32602,
                f"unsupported protocolVersion; expected {STABLE_PROTOCOL_VERSION}",
            )
        return {
    ...
    def _call_tool(self, params: object) -> dict[str, Any]:
        ...
        try:
            ...
            return self._tool_result(
                {"error": "pathway_not_found", "pathway_id": str(exc.args[0])},
                is_error=True,
            )
        except (ValueError, ReadOnlyDatabaseError) as exc:
            return self._tool_result(
    ...
    def _invoke(self, name: str, arguments: Mapping[str, Any]) -> Any:
        if name == "rpr.get_status":
            _require_keys(arguments, allowed=set())
            return self.read_model.status()
        if name == "rpr.list_pathways":
            _require_keys(arguments, allowed={"limit"})
            return {"pathways": self.read_model.list_pathways(limit=arguments.get("limit", 100))}
        if name == "rpr.get_pathway":
            _require_keys(arguments, allowed={"pathway_id"}, required={"pathway_id"})
            return self.read_model.get_pathway(arguments["pathway_id"])
        if name == "rpr.get_evidence":
            _require_keys(
                arguments,
                allowed={"pathway_id", "limit"},
                required={"pathway_id"},
            )
            return {
                "pathway_id": arguments["pathway_id"],
                "events": self.read_model.get_evidence(
                    arguments["pathway_id"], limit=arguments.get("limit", 500)
                ),
            }
        if name == "rpr.list_unresolved":
            _require_keys(arguments, allowed={"limit"})
            return {"pathways": self.read_model.list_unresolved(limit=arguments.get("limit", 100))}
...
def _require_keys(
    ...
    *,
    ...
    if unexpected or missing:
        details: list[str] = []
        if unexpected:
            details.append(f"unexpected arguments: {', '.join(unexpected)}")
        if missing:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="rpr-mcp")
    parser.add_argument("--database", type=Path, required=True, help="Existing RPR SQLite database")
    args = parser.parse_args(argv)
    try:
        read_model = SQLiteReadModel(args.database)
    except ReadOnlyDatabaseError as exc:
        print(f"rpr-mcp: {exc}", file=sys.stderr)
        return 2
    try:
        return run_stdio(ReadOnlyRprMcpServer(read_model))
    finally:
        read_model.close()
