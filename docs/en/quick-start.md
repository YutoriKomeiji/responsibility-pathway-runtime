...
| 4 | Confirm the CLI surface | `rpr --help` and `rpr-mcp --help` output |
| 5 | Run a local synthetic pathway | Pathway, attempt, route visibility, readback, and final state |
...
Use a disposable directory and synthetic data. Confirm that the host application can register a pathway and authority, reach a configured Human Gate when required, retain an execution attempt, inspect Responsibility Routing without inferring Authority, attach independent readback evidence, restore unresolved state after restart, and expose repair or reconciliation when completion cannot be established.
...
| Evidence | Sanitized logs, readback source, route classification, final pathway state |
...
Stop the rehearsal when Authority is absent, receiver eligibility is unknown for the proposed route, independent readback is unavailable, credentials may be exposed, an external effect is ambiguous, the pathway cannot be restored, or the next action would be irreversible without explicit human approval.
