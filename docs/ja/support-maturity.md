...
| Responsibility Routing / bounded Human Gate / repair / resume / reconciliation | Supported source preview | Routingはadditiveに実装・test済みです。公開済み`0.1.0a5` packageはroute visibility追加前のreleaseです。 |
...
| Read-only `rpr-mcp` inspection server | Supported reference | Local trusted client向けinspection surfaceです。現行sourceはread-only route visibilityを追加しています。 |
...
現行sourceのResponsibility Routingは、bounded Human Returnをneutral hold、reconciliation hold、明示的delegation内のeligible receiver、stop/preserve outcomeから区別します。Route recordはreceiver eligibility、delegation scope、unresolved payload、allowed next actions、closure/reevaluation condition、Residual Ownerを保持します。
...
- **Supported source preview** — repository sourceでは実装・test済みですが、独立したreleased package contractとしてはまだpromoteしていません。
