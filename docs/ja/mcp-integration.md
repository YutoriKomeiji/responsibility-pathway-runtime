...
Responsibility Pathway Runtime（RPR）は、統合ApplicationからMCP Serverへ送るTool Callを、責任経路の中で管理できます。現在のPublic Alphaでは、RPRはMCP Serverの手前に置くClient側の実行・証拠レイヤーとして動きます。

> **現在の境界:** RPRはMCP Serverへの呼び出しを統治できます。ただし、RPR自身のPathway操作をMCP Toolとして公開する「RPR MCP Server」は、まだ提供していません。

## 現在できること

現行のMCP経路には、次が実装されています。
...
Public Alphaでは、Local MCP subprocessとstdio経路、Fault Injection、Restart後の継続、Duplicate Dispatch防止を、確認環境内で検証しています。
...
- 最終リハーサルのLinux / Python 3.11以外のOS、Container、Python環境
- ProductionでのBypass防止、監視、Incident Owner、Deployment適合性

## 統合側が担うこと

RPRは、任意のMCP Serverが信頼できると自動判定しません。統合Applicationは、次を設計・運用します。

- MCP Serverの選定と認証
- Credentialと環境変数の保護
- Server Process、Network、Filesystem、Tool Permissionの制限
- どのToolにHuman Gateを要求するか
...
現在のReleaseは、RPR自身をMCP Serverとして公開しません。`rpr.get_pathway`、`rpr.request_human_gate`、`rpr.reconcile`のようなToolは将来の候補であり、`0.1.0a2`の機能ではありません。
