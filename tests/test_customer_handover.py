...
from rpr.customer_handover import CustomerHandoverError, build_customer_handover_candidate, write_customer_handover_candidate
...
def test_handover_candidate_covers_operations_audit_and_maintenance() -> None:
    candidate = build_customer_handover_candidate(source_commit="a" * 40, residual_owner="Akihisa Ono")
...
def test_handover_candidate_rejects_missing_evidence_reference(tmp_path: Path) -> None:
    candidate = build_customer_handover_candidate(source_commit="b" * 40, residual_owner="owner")
    operations = replace(candidate.roles["operations"], evidence=("missing/customer-runbook.md",))
...
def test_handover_candidate_cannot_self_approve() -> None:
    candidate = build_customer_handover_candidate(source_commit="c" * 40, residual_owner="owner")
...
def test_handover_candidate_requires_all_safety_boundaries() -> None:
    candidate = build_customer_handover_candidate(source_commit="d" * 40, residual_owner="owner")
    with pytest.raises(CustomerHandoverError, match="must remain active"):
        replace(candidate, boundaries={**candidate.boundaries, "human_gate_required": False}).to_dict(repository_root=_repository_root())
...
def test_write_handover_candidate_retains_digest_and_source_commit(tmp_path: Path) -> None:
    ...
    document = write_customer_handover_candidate(
        repository_root=_repository_root(), output=output, source_commit="e" * 40, residual_owner="Akihisa Ono"
    )
