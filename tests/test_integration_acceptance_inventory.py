...
INVENTORY = ROOT / "specs/integration-acceptance-inventory-v1.json"
...
def test_inventory_retains_verified_evidence_and_allows_candidate_freeze_review() -> None:
    ...
    assert document["summary"] == {"blocked": 1, "partial": 0, "planned": 0, "verified": 8}
    assert document["candidate_freeze_allowed"] is True
    assert len(document["inventory_sha256"]) == 64
    assert {scenario["next_phase"] for scenario in document["scenarios"]} == {
        "field_evidence", "replacement_freeze", "external_test"
    }
...
def test_external_environment_scenarios_remain_blocked(tmp_path: Path) -> None:
    document = json.loads(INVENTORY.read_text(encoding="utf-8"))
    scenario = next(item for item in document["scenarios"] if item["execution_class"] == "external_environment_only")
