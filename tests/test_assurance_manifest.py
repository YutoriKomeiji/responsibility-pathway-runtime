...
from rpr.assurance_manifest import (
    AssuranceManifestError,
    load_assurance_manifest,
...
def test_repository_manifest_is_valid_and_anchored() -> None:
    ...
    assert loaded["release_boundary"] == "private_rpp_development_only"
...
def test_rejects_public_release_boundary() -> None:
    payload = _manifest()
    payload["release_boundary"] = "public_release"

    with pytest.raises(AssuranceManifestError, match="release_boundary_must_remain_private"):
        validate_assurance_manifest(payload)


def test_rejects_missing_human_gate() -> None:
    payload = _manifest()
    payload["human_gate_required_for"].remove("independent_rpr_export")
...
def test_rejects_unverified_claim_without_blocker() -> None:
    payload = _manifest()
    rc_claim = next(claim for claim in payload["claims"] if claim["claim_id"] == "RPR-RC-01")
    rc_claim["blocked_by"] = []
...
def test_rejects_missing_source_document(tmp_path: Path) -> None:
    ...
    with pytest.raises(AssuranceManifestError, match="source_document_missing"):
        validate_assurance_manifest(payload, root=tmp_path)
