...
def test_baseline_preserves_verified_delivery_and_routes_to_documentation_review() -> None:
    matrix = build_delivery_acceptance_baseline(source_commit="a" * 40, residual_owner="Akihisa Ono")
...
def test_every_baseline_evidence_reference_exists_in_repository() -> None:
    ...
    matrix = build_delivery_acceptance_baseline(source_commit="b" * 40, residual_owner="owner")
    missing = [
        reference
...
def test_baseline_serialization_keeps_documentation_and_human_boundary_visible() -> None:
    matrix = build_delivery_acceptance_baseline(source_commit="c" * 40, residual_owner="owner")
    ...
    assert "src/rpr/customer_handover.py" in document["items"]["customer_handover"]["evidence"]
